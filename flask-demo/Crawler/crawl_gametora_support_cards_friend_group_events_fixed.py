...
- Có --support-type để dùng lại cho speed/stamina/power/guts/wit/friend/group.
- Friend/Group chỉ lấy dữ liệu Global và đọc đủ Dates, Random Events, Special Events.
- Card thường vẫn đọc Chain Events và Random Events.

Cài lần đầu:
    pip install playwright requests
    python -m playwright install chromium

Crawl thử vài card Friend theo cách lọc thủ công trên GameTora:
    python crawl_gametora_support_cards_friend_group_events_fixed.py --manual-filter --support-type friend --limit 3

Sau khi test ổn, crawl toàn bộ Friend:
    python crawl_gametora_support_cards_friend_group_events_fixed.py --manual-filter --support-type friend --scan-filtered-list

Crawl Group GLOBAL và gộp vào 3 file CSV gốc:
    python crawl_gametora_support_cards_friend_group_events_fixed.py --manual-filter --support-type group --scan-filtered-list --append

Sau này chỉ cần đổi --support-type:
    python crawl_gametora_support_cards_friend_group_events_fixed.py --manual-filter --support-type stamina
    python crawl_gametora_support_cards_friend_group_events_fixed.py --manual-filter --support-type power
    python crawl_gametora_support_cards_friend_group_events_fixed.py --manual-filter --support-type guts
    python crawl_gametora_support_cards_friend_group_events_fixed.py --manual-filter --support-type wit
    python crawl_gametora_support_cards_friend_group_events_fixed.py --manual-filter --support-type friend
    python crawl_gametora_support_cards_friend_group_events_fixed.py --manual-filter --support-type group
"""
...
def normalize_support_type(value):
    ...
    # GameTora đôi lúc gọi Friend là Pal. Chuẩn hóa về friend.
    if value == "pal":
        return "friend"
...
SUPPORT_EFFECT_NAMES = sorted(
    [
        ...
        "Wit Bonus",
        "Skill Point Bonus",
        # Tên hiện tại trên GameTora của Friend/Group cards.
        "Event Recovery",
        "Event Effectiveness",
        "Failure Protection",
        "Energy Cost Reduction",
        # Giữ các tên cũ để crawler vẫn tương thích nếu GameTora đổi label.
...
# Các nhóm event có thể xuất hiện trên trang support detail.
# - Card thường: Chain Events + Random Events.
# - Friend/Group: Dates (tương đương chuỗi chain), Random Events + Special Events.
# Dùng section riêng "date" để CSV phân biệt rõ Dates với Chain Events thường.
...
TRAINING_EVENT_GROUP_TITLES = [group_title for _, group_title in TRAINING_EVENT_GROUPS]
TRAINING_EVENT_STOP_TITLES = TRAINING_EVENT_GROUP_TITLES + [
    "Character Rate Up",
    "Support Rate Up",
    "Upcoming Characters",
    "Upcoming Supports",
...
def collect_support_links_by_type(page, required_type, viewport_only=False):
    """
    Tự nhận diện Friend/Group trên trang list.

    Bản sửa:
    - Chỉ xét card box đang render thật; không lấy card ẩn của tab/server khác.
    - Không còn nhận Group chỉ vì URL kết thúc bằng ``-group``. Quy tắc cũ chính là
      nguyên nhân crawler ưu tiên một số Group JP thay vì danh sách Global đang lọc.
    - Có thể giới hạn trong viewport hiện tại để dùng khi quét list đã lọc.

    Khi chạy ``--manual-filter``, main() sẽ dùng chính danh sách Global + Type mà
    người dùng đang nhìn thấy. Hàm này chủ yếu là fallback cho chế độ không manual.
    """
    required_type = normalize_support_type(required_type)

    return page.evaluate(
        r"""
        ({ requiredType, friendMarkers, groupMarkers, viewportOnly }) => {
            const normalize = (value) => String(value || '')
                .toLowerCase()
                .replace(/[^a-z0-9]+/g, ' ')
                .trim();

            const exactTypeToken = (text, values) => {
            ...
            const isRendered = (el) => {
                ...
                // offsetParent null thường là card nằm trong panel/tab đã bị ẩn.
                // Giữ position:fixed vì offsetParent của nó có thể null hợp lệ.
            ...
            const validSupportHref = (href) => {
                try {
                    const url = new URL(href, location.origin);
                    return /\/umamusume\/supports\/\d+-[^/?#]+\/?$/i.test(url.pathname);
                } catch (_) {
                    return false;
                }
            };

            const getCardRoot = (anchor) => {
                let best = anchor;
                let current = anchor;

                while (current.parentElement && current.parentElement !== document.body) {
                    const parent = current.parentElement;
                    const supportLinkCount = parent.querySelectorAll(
                        'a[href*="/umamusume/supports/"]'
                    ).length;

                    if (supportLinkCount > 1) break;

                    const r = parent.getBoundingClientRect();
                    const hasImage = parent.querySelector('img, picture, svg');
                    ...
                    if (
                        ...
                        r.height < Math.max(window.innerHeight * 1.5, 1300)
                    ) {
                        best = parent;
                    }
            ...
            const getCardSearchText = (root) => {
                const nodes = [
                    root,
                    ...Array.from(root.querySelectorAll(
                        'img, source, svg, use, [data-type], [data-support-type], [aria-label], [title]'
                    )),
                ];
                ...
                return nodes.map(el => [
                    ...
                    el.getAttribute && el.getAttribute('xlink:href'),
                    el.getAttribute && el.getAttribute('alt'),
                    el.getAttribute && el.getAttribute('title'),
                    el.getAttribute && el.getAttribute('aria-label'),
                    el.getAttribute && el.getAttribute('data-type'),
                    el.getAttribute && el.getAttribute('data-support-type'),
                    typeof el.className === 'string' ? el.className : '',
                ].filter(Boolean).join(' ')).join(' ').toLowerCase();
            };

            const anchors = Array.from(
                document.querySelectorAll('a[href*="/umamusume/supports/"]')
            );
            const results = [];
            const seen = new Set();
            ...
            for (const anchor of anchors) {
                ...
                if (!validSupportHref(rawHref)) continue;

                const href = new URL(rawHref, location.origin).href;
                if (seen.has(href)) continue;

                const root = getCardRoot(anchor);

                // Quan trọng: loại card thuộc panel JP/tab ẩn khỏi kết quả.
                if (!isRendered(root)) continue;
                if (viewportOnly && !intersectsViewport(root)) continue;

                const searchText = getCardSearchText(root);
                let matches = false;

                if (requiredType === 'friend') {
                    const hasFriendMarker = friendMarkers.some(marker =>
                        searchText.includes(String(marker).toLowerCase())
                    );
                    const hasFriendType = exactTypeToken(searchText, ['friend', 'pal']);
                    matches = hasFriendMarker || hasFriendType;
                } else if (requiredType === 'group') {
                    const hasGroupMarker = groupMarkers.some(marker =>
                        searchText.includes(String(marker).toLowerCase())
                    );
                    const hasGroupType = exactTypeToken(searchText, ['group']);

                    // Tuyệt đối không dùng slug URL ``...-group`` làm bằng chứng type nữa.
                    matches = hasGroupMarker || hasGroupType;
                } else {
                    matches = true;
                }
        ...
        """,
        {
            ...
            "friendMarkers": list(FRIEND_TYPE_ICON_MARKERS),
            "groupMarkers": [
                "support_group",
                "support-group",
                "support group",
...
def collect_filtered_links_by_scrolling(page, required_type="", max_rounds=80):
    """
    Cuộn list đã được người dùng lọc và gom card theo từng viewport.

    Với ``--manual-filter``, chính trạng thái Global + Type trên browser là nguồn
    sự thật. Vì vậy không quét toàn DOM và không tự suy đoán JP/Global nữa.
    """
    required_type = normalize_support_type(required_type)
    links = []
    seen = set()
    ...
    for _ in range(max_rounds):
        if required_type in ["friend", "group"]:
            current_links = collect_support_links_by_type(
                page,
                required_type,
                viewport_only=True,
            )

            # Một số bản GameTora không để type trong alt/class của card list.
            # Khi người dùng đã lọc thủ công, fallback sang mọi card visible trong viewport.
            if not current_links:
                current_links = collect_support_links(
                    page,
                    visible_only=True,
                    viewport_only=True,
                )
...
def guess_support_type(page):
    """
    Đọc đúng type của card, đặc biệt tránh nhầm "Friendship Bonus" thành Friend.

    Ưu tiên vùng ngay sau label Type, sau đó mới dùng meta/ảnh toàn trang làm fallback.
    """
    return page.evaluate(
        r"""
        () => {
            ...
            const normalizeType = (value) => {
                ...
                // Chỉ nhận token nguyên vẹn. Nhờ vậy "friendship" không bị hiểu thành "friend".
                for (const token of tokens) {
                    if (!supportedTypes.has(token)) continue;
                    return token === 'intelligence' ? 'wit' : token;
                }

                // Meta description của GameTora thường có dạng "(SSR, Friend) support card".
                const metaMatch = raw.match(/\((?:ssr|sr|r)\s*,\s*(speed|stamina|power|guts|wit|intelligence|friend|group)\)/i);
                if (metaMatch) {
                    return metaMatch[1].toLowerCase() === 'intelligence'
                        ? 'wit'
            ...
            const typeLabels = all.filter(el => exactText(el).toLowerCase() === 'type');
            ...
            for (const typeLabel of typeLabels) {
                ...
                const candidates = all
                    .map(el => {
                        const r = el.getBoundingClientRect();
                        const visibleText = exactText(el).toLowerCase();
                        const exactVisibleType = supportedTypes.has(visibleText)
                            ? (visibleText === 'intelligence' ? 'wit' : visibleText)
                            : '';
                        ...
                        const attributes = [
                            ...
                            el.getAttribute('data-support-type'),
                            el.className,
                        ].join(' ');

                        const imgs = el.querySelectorAll
                            ? Array.from(el.querySelectorAll('img, svg image')).map(img => [
                                img.alt,
                                img.title,
                                img.src,
                        ...
                        return {
                            type: exactVisibleType || normalizeType(attributes + ' ' + imgs),
                            top: r.top,
                            left: r.left,
                            distance: Math.abs(r.top - labelRect.top) + Math.abs(r.left - labelRect.right),
                            area: r.width * r.height,
                        };
                    })
                    .filter(item => item.type)
                    .filter(item => item.top >= labelRect.top - 80 && item.top <= labelRect.bottom + 180)
            ...
            const structuredText = Array.from(document.querySelectorAll('script[type="application/ld+json"]'))
                .map(script => script.textContent || '')
                .join(' ');
            type = normalizeType(structuredText);
            if (type) return type;
...
def is_global_support_detail(page):
    """
    Trả về False khi GameTora đang hiển thị dữ liệu JP cho card chưa ra Global.

    GameTora ghi rõ trên detail:
    ``This support card is not yet released on the Global server. Showing JP server data.``
    Kiểm tra này là lớp bảo vệ cuối cùng, kể cả khi list filter/DOM thay đổi.
    """
    try:
        result = page.evaluate(
            ...
            () => {
                ...
                const jpMarkers = [
                    'not yet released on the global server',
                    'showing jp server data',
                    'jp server data',
                    'not released on global',
                ];

                return !jpMarkers.some(marker => text.includes(marker));
            }
            """
        )
        return bool(result)
    except Exception:
        # Không xác định được thì không tự ý loại card; type guard vẫn còn phía sau.
        return True
...
def scroll_training_group_into_view(page, group_title):
    """Kéo đúng heading event vào viewport để GameTora render/lazy-load các nút bên dưới."""
    try:
        return page.evaluate(
            r"""
            ({ groupTitle }) => {
                ...
                const wanted = normalize(groupTitle);
                const candidates = Array.from(document.querySelectorAll('body *'))
                    .filter(el => normalize(el.innerText || el.textContent) === wanted)
                    .map(el => {
                        const r = el.getBoundingClientRect();
                        return { el, area: r.width * r.height, top: r.top + window.scrollY };
                    })
...
def get_training_event_button_titles(page, group_title):
    """
    Lấy title event trong đúng nhóm Dates / Chain / Random / Special.

    Bản sửa hỗ trợ cả button, link và phần tử click được; đồng thời so khớp
    heading không phân biệt hoa thường. Điều này khắc phục Special Events bị bỏ
    qua khi GameTora render item bằng anchor/div thay vì button chuẩn.
    """
    scroll_training_group_into_view(page, group_title)
    page.wait_for_timeout(500)

    return page.evaluate(
        r"""
        ({ groupTitle, allGroupTitles, stopTitles }) => {
            const normalize = value => String(value || '')
                .replace(/\u00a0/g, ' ')
                .replace(/\s+/g, ' ')
                .trim()
            ...
            const exact = el => clean(el.innerText || el.textContent || '');
            const wantedGroup = normalize(groupTitle);
            const normalizedGroups = new Set(allGroupTitles.map(normalize));
            const all = Array.from(document.querySelectorAll('body *')).filter(isRendered);

            const groups = all
                .filter(el => normalize(exact(el)) === wantedGroup)
                .map(el => {
                    const r = el.getBoundingClientRect();
                    return { el, top: r.top + window.scrollY, area: r.width * r.height };
                })
                .sort((a, b) => a.area - b.area || a.top - b.top);

            if (!groups.length) return [];

            const groupTop = groups[0].top;
            let stopTop = Number.POSITIVE_INFINITY;

            for (const title of stopTitles) {
                if (normalize(title) === wantedGroup) continue;
                const wantedStop = normalize(title);

                const candidates = all
                    .filter(el => normalize(exact(el)) === wantedStop)
                    .map(el => {
                        const r = el.getBoundingClientRect();
                        return { top: r.top + window.scrollY, area: r.width * r.height };
                    })
                    .filter(item => item.top > groupTop + 5)
                    .sort((a, b) => a.top - b.top || a.area - b.area);

                if (candidates.length) stopTop = Math.min(stopTop, candidates[0].top);
            }

            if (!Number.isFinite(stopTop)) stopTop = document.body.scrollHeight;

            const rawCandidates = Array.from(document.querySelectorAll(
                'button, [role="button"], a, [onclick], [tabindex], div, span'
            ));
            ...
            const items = rawCandidates
                ...
                .map(el => {
                    ...
                    const clickable = tag === 'button' || tag === 'a' || role === 'button'
                        ...
                        || className.includes('button') || className.includes('event');
                ...
                .filter(item => item.top > groupTop + 2 && item.top < stopTop - 2)
                .filter(item => item.clickable)
                .filter(item => item.text.length > 0 && item.text.length <= 220)
                .filter(item => item.lineCount <= 2)
                .filter(item => !normalizedGroups.has(normalize(item.text)))
                .filter(item => !['details', 'more'].includes(normalize(item.text)))
                .sort((a, b) => a.top - b.top || a.area - b.area);
        ...
        {
            "groupTitle": group_title,
            "allGroupTitles": TRAINING_EVENT_GROUP_TITLES,
            "stopTitles": TRAINING_EVENT_STOP_TITLES,
        },
    )

def click_event_button(page, group_title, title):
    """Click đúng event trong đúng nhóm, hỗ trợ button/link/div click được."""
    scroll_training_group_into_view(page, group_title)
    page.wait_for_timeout(250)

    return page.evaluate(
        r"""
        ({ groupTitle, title, stopTitles }) => {
            const normalize = value => String(value || '')
                .replace(/\u00a0/g, ' ')
                .replace(/\s+/g, ' ')
                .trim()
            ...
            const exact = el => String(el.innerText || el.textContent || '').trim();
            const all = Array.from(document.querySelectorAll('body *')).filter(isRendered);
            const wantedGroup = normalize(groupTitle);
            ...
            for (const stopTitle of stopTitles) {
                if (normalize(stopTitle) === wantedGroup) continue;
                const wantedStop = normalize(stopTitle);
                const candidates = all
                    .filter(el => normalize(exact(el)) === wantedStop)
                    .map(el => {
            ...
            const candidates = raw
                .filter(isRendered)
                .filter(el => normalize(exact(el)) === wantedTitle)
                .map(el => {
                    const r = el.getBoundingClientRect();
                    const top = r.top + window.scrollY;
                    let clickable = el.closest(
                        'button, [role="button"], a, [onclick], [tabindex]'
                    );
                    ...
                    const clickableRect = clickable ? clickable.getBoundingClientRect() : r;
                    return {
                        el: clickable || el,
                        top,
                        area: clickableRect.width * clickableRect.height,
                        valid: Boolean(clickable),
                    };
                })
...
def extract_active_event_text(page, title):
    """
    Lấy panel kết quả của event vừa mở.

    Ưu tiên container có class/id liên quan event/detail/content và loại bỏ các
    container chỉ chứa cả danh sách nút. Giới hạn text được nới rộng cho Special
    Events có note scenario dài.
    """
    return page.evaluate(
        r"""
        ({ title, groupTitles }) => {
            ...
            const candidates = Array.from(document.querySelectorAll('body *'))
                ...
                .map(el => {
                    ...
                    let score = r.width * r.height * 0.001 + text.length * 0.02;
                    if (/event|detail|content|result|choice|training/.test(classText)) score -= 600;
                    if (String(el.tagName).toLowerCase() === 'button') score += 4000;
                    if (String(el.tagName).toLowerCase() === 'a') score += 2500;
                    if (lines.length >= 2) score -= 300;
                    if (lines.length >= 3) score -= 150;

                    const groupHits = normalizedGroups.filter(group =>
                        normalizedText.includes(group)
                    ).length;
                    score += groupHits * 2200;

                    const arrowHits = (text.match(/\(\s*[›❯▶]+\s*\)/g) || []).length;
                    score += Math.max(0, arrowHits - 1) * 1500;
                ...
                .filter(item => item.normalizedText.includes(wanted))
                .filter(item => item.normalizedText !== wanted)
        ...
        """,
        {"title": title, "groupTitles": TRAINING_EVENT_GROUP_TITLES},
    )
...
def event_text_to_detail(raw_text, title):
    ...
    cleaned = []
    normalized_title = clean_line(title)
    clean_title = re.sub(r"^\([^)]*\)\s*", "", normalized_title).strip()

    for line in lines:
        line = clean_line(line)

        # Bỏ title bị lặp trong card event, kể cả khi ❯ đã được chuẩn hóa thành ›.
        if line == normalized_title or line == clean_title:
            continue
        ...
        if line in TRAINING_EVENT_GROUP_TITLES:
            continue
...
def extract_training_events(page, card_id):
    """
    Đọc tất cả nhóm event đang tồn tại trên card.

    Card thường: Chain Events, Random Events.
    Friend/Group: Dates, Random Events, Special Events.

    Bản sửa chủ động scroll từng heading vào viewport, retry việc lấy nút và retry
    panel sau click. Nhờ vậy Special Events ở cuối trang không còn bị lazy-load bỏ qua.
    """
    event_rows = []

    # Đưa Training Events lên gần viewport trước để framework render các section con.
    try:
        scroll_training_group_into_view(page, "Training Events")
        page.wait_for_timeout(600)
    except Exception:
        pass

    for section, group_title in TRAINING_EVENT_GROUPS:
        scroll_training_group_into_view(page, group_title)
        page.wait_for_timeout(550)

        titles = get_training_event_button_titles(page, group_title)

        # Retry một lần sau khi scroll nhẹ; hữu ích với Special Events nằm cuối trang.
        if not titles:
        ...
        if not titles:
            print(f"[EVENT GROUP] {group_title}: 0")
            continue

        print(f"[EVENT GROUP] {group_title}: {len(titles)}")

        for sort_index, title in enumerate(titles, start=1):
            clicked = click_event_button(page, group_title, title)

            if not clicked:
                # Một lần retry sau khi đưa heading trở lại viewport.
                scroll_training_group_into_view(page, group_title)
                page.wait_for_timeout(300)
                clicked = click_event_button(page, group_title, title)

            if not clicked:
                print(f"[EVENT WARN] Không click được {group_title}: {title}")
                continue
            ...
            if not raw_event_text:
                # Có trang cần click lần hai sau khi animation/collapse hoàn tất.
                click_event_button(page, group_title, title)
                page.wait_for_timeout(650)
                raw_event_text = extract_active_event_text(page, title)
            ...
            if not detail:
                print(f"[EVENT EMPTY] {group_title}: {title}")

            # Dù body rỗng vẫn giữ title để không làm mất hẳn Special Event.
            event_rows.append(
                {
                    "card_id": card_id,
                    "section": section,
...
def crawl_one_card(page, url, args, already_open=False):
    ...
    # Friend/Group phải là dữ liệu Global. Nếu detail hiện thông báo đang dùng JP
    # server data thì bỏ ngay, không ghi vào bất kỳ CSV nào.
    if wanted_type in ["friend", "group"] and not is_global_support_detail(page):
        print(f"[SKIP JP] Chưa phát hành Global: {name} | {url}")
        return None
...
def main():
    ...
    parser.add_argument("--support-type", default="speed", choices=["speed", "stamina", "power", "guts", "wit", "pal", "friend", "group", "all"], help="Loại support muốn crawl, gồm cả friend và group")
    ...
    parser.add_argument("--max-links", type=int, default=0, help="Giới hạn an toàn số link Friend/Group; 0 = dùng mặc định theo type")
    ...
    with sync_playwright() as playwright:
        ...
        if args.manual_filter:
            print("\n[MANUAL FILTER — GLOBAL]")
            print("1) Trên browser Playwright, chọn database/server = Global.")
            print(
                "2) Lọc Type = "
                + display_support_type(wanted_type_for_preskip)
                + "."
            )
            print("3) Chờ list cập nhật xong. Crawler chỉ lấy card đang hiện của filter này.")
            input("4) Quay lại Terminal và bấm Enter để bắt đầu... ")

            if args.scan_filtered_list:
                links = unique_keep_order(
                    collect_filtered_links_by_scrolling(
                        page,
                        required_type=wanted_type_for_preskip,
                    )
                )
                print(f"[FOUND GLOBAL] {len(links)} card trong toàn bộ list đã lọc")
            else:
                # Khi chỉ crawl vài card còn thiếu, lấy đúng viewport hiện tại.
                links = unique_keep_order(
                ...
                print(f"[FOUND GLOBAL] {len(links)} card đang nằm trong viewport")

        elif wanted_type_for_preskip in ["friend", "group"]:
            # Chế độ không manual: tự nhận diện type nhưng vẫn chỉ lấy card render thật.
            # crawl_one_card() còn có lớp guard bỏ mọi detail đang hiện JP server data.
            scroll_to_bottom(page, max_rounds=30)
            links = unique_keep_order(
                collect_support_links_by_type(
                    page,
                    wanted_type_for_preskip,
                    viewport_only=False,
                )
            )

            default_safe_limit = 120 if wanted_type_for_preskip == "friend" else 60
            safe_limit = args.max_links if args.max_links > 0 else default_safe_limit

            print(
                f"[TYPE FILTER] {display_support_type(wanted_type_for_preskip)}: "
                f"tìm thấy {len(links)} link đang render"
            )

            if len(links) == 0:
                print(
                    "[STOP] Không nhận diện được card đúng type. "
                    "Hãy chạy lại với --manual-filter, chọn Global + Type trên browser."
                )
                browser.close()
                return
            ...
            if len(links) > safe_limit:
                print(
                    f"[STOP] Có {len(links)} link, vượt giới hạn an toàn {safe_limit}. "
                    "Crawler dừng để tránh quét toàn database."
                )
                browser.close()
                return
        else:
            ...
            links = unique_keep_order(
                collect_support_links(page, visible_only=True, viewport_only=False)
            )
    ...
    if args.append:
        ...
        print(f"python crawl_gametora_support_cards_friend_group_events_fixed.py --manual-filter --support-type {args.support_type} --append")
